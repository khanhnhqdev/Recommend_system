...
def precision_at(predictions, labels, k=10, assume_unique=True):
    ...
    def _inner_pk(pred, lab):
        ...
        if lab.shape[0] > 0:
            ...
            print(np.in1d(pred[:n], lab, assume_unique=assume_unique))
            print(float(cnt) / min(k, lab.shape[0]))
            return float(cnt) / min(k, lab.shape[0])
        else:
